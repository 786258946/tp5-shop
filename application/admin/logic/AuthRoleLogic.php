 ...
 * @package admin\Logic
 */
class AuthRoleLogic extends Model
{
     ...
     * @return  mix
     */
    public function auth_role_list($id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        static $res = NULL;
        
        if ($res === NULL)
        {
            $data = extra_cache('admin_auth_role_list_logic');
            if ($data === false)
            {
                $where = array(
                    'c.status' => 1,
                ...
                $res = DB::name('auth_role')
                    ->field($fields)
                    ->alias('c')
                    ->join('__AUTH_ROLE__ s','s.pid = c.id','LEFT')
                    ->where($where)
                    ->group('c.id')
                    ->order('c.pid asc, c.id')
                    ->select();
                extra_cache('admin_auth_role_list_logic', $res);
            }
            else
            {
                $res = $data;
        ...
        $options = $this->auth_role_options($id, $res); // 获得指定分类下的子分类的数组
     ...
     * @return  void
     */
    public function auth_role_options($spec_id, $arr)
    {
